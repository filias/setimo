...
(async function () {
  ...
  document.title = `${subject.name} — 8.º ano — Oitavo`;
  content.innerHTML = '';
...
function buildHero(subject) {
  ...
  return el('section', { class: 'subject-hero' }, [
    el('div', { class: 'wrap' }, [
      el('p', { class: 'breadcrumb', html: `<a href="index.html">8.º ano</a> › ${esc(subject.name)}` }),
...
function initModes() {
  ...
  const saved = localStorage.getItem('oitavo-mode') || 'both';

  function apply(mode) {
    document.body.dataset.mode = mode;
    localStorage.setItem('oitavo-mode', mode);
